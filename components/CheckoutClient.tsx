...
export default function CheckoutClient({ user, bump, rewardPercent }: Props) {
          ...
          {(() => {
            ...
            return (
              <div className="mt-4 flex flex-col gap-2 border-t border-line pt-4 text-sm">
                ...
                {(() => {
                  const estimatedPoints = calculateEarnedPoints(effSubtotal, rewardPercent);
                  if (estimatedPoints <= 0) return null;
                  return (
                    <p className="tnum mt-1 flex items-center gap-1.5 text-sm font-medium text-amber-400">
                      <span aria-hidden="true">🎁</span>
                      هتكسب تقريبًا +{estimatedPoints} نقطة بعد استلام الطلب
                    </p>
                  );
                })()}
              </div>

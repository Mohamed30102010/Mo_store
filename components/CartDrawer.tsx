...
export default function CartDrawer({ rewardPercent = 0 }: { rewardPercent?: number }) {
  const {
    items,
    count,
    subtotalCents,
  ...
  const estimatedPoints = calculateEarnedPoints(subtotalCents, rewardPercent);
            ...
            <div className="border-t border-line bg-surface/40 px-5 py-4">
              ...
              {estimatedPoints > 0 && (
                <p className="tnum mt-2 flex items-center gap-1.5 text-sm font-medium text-amber-400">
                  ...
                  هتكسب تقريبًا +{estimatedPoints} نقطة
                </p>
              )}
              <Link
                href="/checkout"

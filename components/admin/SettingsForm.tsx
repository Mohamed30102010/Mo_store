...
export default function SettingsForm({ settings, pixelStates, products }: Props) {
  ...
  return (
    <form action={formAction} className="flex flex-col gap-6">
      ...
      {/* ═══ نقاط المكافآت ═══ */}
      <section className="rounded-2xl border border-line bg-surface p-5">
        <h3 className="mb-1 font-bold text-fg">🎁 نقاط المكافآت (Rewards / نقاطي)</h3>
        <p className="mb-4 text-sm text-muted">
          نسبة النقاط اللي بتتمنح للعميل من قيمة المنتجات (مش شامل الشحن)، وبتُحسب فقط لما
          الطلب يوصل لحالة <b className="text-fg">تم التسليم</b> — ومش بتتمنح للطلبات
          الملغاة أو المرتجعة.
        </p>
        <div className="max-w-xs">
          <label className="mb-1 block text-sm font-semibold text-fg">نسبة المكافآت</label>
          <div className="flex items-center gap-2">
            <input
              type="number"
              name="reward_percent"
              defaultValue={settings.reward_percent || "30"}
              min={0}
              max={100}
              step={1}
              dir="ltr"
              className="w-28 rounded-lg border border-line bg-bg px-3 py-2 text-sm text-fg focus:border-brand-500 focus:outline-none"
            />
            <span className="text-sm text-muted">%</span>
          </div>
          <p className="mt-1.5 text-xs text-muted">
            مثال: 100 ج.م مشتريات مؤهّلة عند 30% = 30 نقطة.
          </p>
        </div>
      </section>
